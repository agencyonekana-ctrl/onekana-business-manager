...
export default function Invoices() {
  ...
  return (
    <div className="space-y-6">
      ...
      {error && <EmptyState title="Donnees indisponibles" description="Les factures et paiements seront affiches des qu ils seront disponibles." />}
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card className="border-primary/15 bg-white">
          ...
          <CardContent className="p-0">
            {loading ? <div className="p-6 text-center text-sm text-muted-foreground">Chargement...</div> :
              invoices.length === 0 ? <div className="p-6"><EmptyState title="Aucune facture" description="Aucune facture n est encore disponible." /></div> :
        ...
        <Card className="border-primary/15 bg-white">
          ...
          <CardContent className="p-0">
            {loading ? <div className="p-6 text-center text-sm text-muted-foreground">Chargement...</div> :
              payments.length === 0 ? <div className="p-6"><EmptyState title="Aucun paiement" description="Aucun paiement n est encore disponible." /></div> :
